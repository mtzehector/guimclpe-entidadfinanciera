...
@Provider
public class EstatusConciliacionPersistenciaService extends ServiceDefinition<EstatusConciliacionRequest, EstatusConciliacionRequest> {
    ...
    public Message<EstatusConciliacionRequest> execute(Message<EstatusConciliacionRequest> request) throws BusinessException {
        try {
            Response respuesta = estatusClient.guardarEstatusConciliacion(request.getPayload());
            if (respuesta != null && respuesta.getStatus() == 200){
                EstatusConciliacionRequest estatus = respuesta.readEntity(EstatusConciliacionRequest.class);
                request.getPayload().setId(estatus.getId());
                request.getPayload().setPeriodo(estatus.getPeriodo());
                request.getPayload().setActivo(estatus.getActivo());
                return request;
            }
        }catch (Exception e){
            log.log(Level.SEVERE, "ERROR EstatusConciliacionPersistenciaService.execute() = {0}", e);
        }
        return response(
                null,
