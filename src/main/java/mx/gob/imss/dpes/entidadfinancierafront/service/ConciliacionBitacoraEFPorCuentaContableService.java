...
@Provider
public class ConciliacionBitacoraEFPorCuentaContableService extends ServiceDefinition<ConciliacionRequest, ConciliacionRequest> {
        ...
        try {
            ...
            for (Long cveEntidadFinanciera : entidades) {
                ...
                if (respuesta != null && respuesta.getStatus() == 204) {
                    ...
                    Documento documento = this.obtenerDocumentoPorId(documentoConciliacion.getDocumento().getId());
                    this.agregarDocumento(documento, request.getPayload());
                }
            }
    ...
    private Documento obtenerDocumentoPorId(Long id){
        Response respuesta = documentoClient.obtenerDocumentoPorId(id);
        if (respuesta != null && respuesta.getStatus() == 200)
            return respuesta.readEntity(Documento.class);

        return null;
    }

    private void agregarDocumento(Documento doc, ConciliacionRequest req){

        if (doc != null){
            if (req.getCveTipoDocumento().equals(TipoDocumentoEnum.SOLICITUD_TRANSFERENCIA_RECURSOS_RETENCIONES.getTipo())) {
                req.getListSolicitudTransferencia().add(doc);
            }

            if (req.getCveTipoDocumento().equals(TipoDocumentoEnum.REPORTE_RETENCIONES_EF_DESGLOSADO_CUENTA_CONTABLE.getTipo())) {
                req.getListCuentaContable().add(doc);
            }
        }
