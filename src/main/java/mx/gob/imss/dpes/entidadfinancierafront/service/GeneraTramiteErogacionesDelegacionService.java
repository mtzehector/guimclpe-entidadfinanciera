...
@Provider
public class GeneraTramiteErogacionesDelegacionService extends ServiceDefinition<ConciliacionRequest, ConciliacionRequest> {
    ...
    public Message<ConciliacionRequest> execute(Message<ConciliacionRequest> request) throws BusinessException {
        try {
            request.getPayload().setCveTipoDocumento(
                    TipoDocumentoEnum.REPORTE_RETENCIONES_DELEGACION.getTipo()
            );

            Response respuesta = tramiteErogacionesClient.generaReporteRetencionPorDelegacion(request.getPayload());
            ...
            log.log(Level.SEVERE, "ERROR GeneraTramiteErogacionesDelegacionService.execute() = {0}", e);
