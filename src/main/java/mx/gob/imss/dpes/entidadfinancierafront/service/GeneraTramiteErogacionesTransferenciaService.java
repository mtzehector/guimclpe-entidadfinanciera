...
@Provider
public class GeneraTramiteErogacionesTransferenciaService extends ServiceDefinition<ConciliacionRequest, ConciliacionRequest> {
    ...
    public Message<ConciliacionRequest> execute(Message<ConciliacionRequest> request) throws BusinessException {
        try {
            request.getPayload().setCveTipoDocumento(
                    TipoDocumentoEnum.SOLICITUD_TRANSFERENCIA_RECURSOS_RETENCIONES.getTipo()
            );
            //request.getPayload().setErogacion(true);

            Response respuesta = tramiteErogacionesClient.generaReporteSolicitudTransferencia(request.getPayload());
            ...
            log.log(Level.SEVERE, "ERROR GeneraReporteTramiteErogacionesService.execute() = {0}", e);
